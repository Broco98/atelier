...
// 같다」) 몸통이 하나여야 한다 — 라우트 파일마다 적으면 한쪽만 고친 날 Maison에서만 초안이
...
export async function pickWorkSlug(mode: Mode, queryClient: QueryClient) {
  ...
  return pickSlug(shellStore.state.workSlug[mode], works, isDefaultSelectable);
}

/** 아카이브에는 `isPreferred`가 없다 — 치운 것에 초안이라는 상태가 없다. */
