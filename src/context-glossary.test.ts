...
describe("말의 사전", () => {
  ...
  it("「모든 프로젝트」가 등재돼 있고, 폴더 이름과 「루트」를 거부한다", () => {
    // 결정 17~19(티켓 08). 멀티 프로젝트 work에서 ⌘T가 여는 자리에 이름이 없으면, 다음 사람이
    // 폴더 이름(`trees`)이나 「루트」로 부른다 — 앞은 코어의 구현 이름이고 뒤는 데이터 루트
    // (최상위 터미널의 자리)와 겹친다. 거부 목록에 들어야 접근성 이름 검사(위)가 그 낱말을 문다.
  ...
  it("「레이아웃」·「항목」·「템플릿」·「번호 묶음」이 등재돼 있다", () => {
    ...
    // 낱말로 섞는다.
    expect(names).toEqual(expect.arrayContaining(["레이아웃", "항목", "템플릿", "번호 묶음"]));
  });

  it("「레이아웃」이 「노드」·「화면 배치」를 거부하고, 템플릿과 가른다", () => {
    // 두 말은 자기 항목이 없어 거부어가 된다 — 위 접근성 이름 검사가 편집기의 `aria-label`에서 그
    // 낱말을 문다. 「템플릿」은 자기 항목이라 거부어가 아니다(`aria-label="템플릿 …"`이 옳다).
    expect(refused).toEqual(expect.arrayContaining(["노드", "화면 배치"]));
    expect(refused).not.toContain("템플릿");
    expect(bodyOf("레이아웃")).toContain("레이아웃을 템플릿이라 부르지 않는다");
  });

  it("「항목」이 설정 nav의 줄을 「설정 nav 항목」으로 가르고, 맨 위를 「루트」라 부르지 않는다", () => {
    // develop 사전은 설정 nav의 줄도 「항목」이라 불렀다 — 「패널 탭」처럼 수식으로 가른다.
    expect(bodyOf("항목")).toContain("설정 nav 항목");
    expect(bodyOf("항목")).toContain("맨 위 항목");
    expect(refused).toContain("루트");
  });
...
// 설정에 들어가면 사이드바가 설정 nav로 바뀐다(UI개선 결정 21). 그 화면에는 세그먼트가 없어서
// 「세그먼트가 모드를 말한다」·「설정에서 세그먼트가 떠나온 모드를 켠다」가 거짓이 됐다.
describe("사전이 설정의 사실과 맞는다", () => {
  it("「모드」가 설정에서 세그먼트를 말하지 않는다", () => {
    expect(bodyOf("모드")).not.toContain("세그먼트가 켜는 것은");
    expect(bodyOf("모드")).toContain("설정에는 세그먼트가 없고");
    expect(bodyOf("모드")).toContain("떠나온 모드 하나");
  });

  // **한 줄 안에서** 잰다 — 절 전체로 보면 이웃한 「문장은 한국어」가 「한국어」를 이미 들고 있어
  // 설정 nav 줄이 빠져도 초록이 된다(위 `uppercase` 주석이 든 그 샘).
  it("표기 절이 설정 nav의 라벨을 한국어로 든다", () => {
    expect(notation).toMatch(/설정 nav[^\n]*한국어/);
  });

  // 넷째 줄(spec 레이아웃 티켓 08). 라벨 나열이 지금 화면의 넷을 다 든다 — 소문자 `spec`은 「spec 폴더」와
  // 같은 쓰임이라 대문자 층도 탭 가족도 아니다.
  it("표기 절의 설정 nav 라벨 나열에 `spec 레이아웃`이 있다", () => {
    expect(notation).toMatch(/설정 nav[^\n]*`에이전트 훅`·`spec 레이아웃`/);
  });
});
