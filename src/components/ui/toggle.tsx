// 앱 규격으로 고친 자리: hover bg-muted·text-foreground(3%, 행 hover 농도)→quiet-hover(state-2, 버튼 hover), 눌림 aria-pressed:bg-muted·data-[state=on]:bg-muted→aria-pressed:toggle-on(state-3 — 켜짐), 글자 text-sm→13.5px(sm 0.8rem→12.5px, Button과 같다). 안 눌린 글자는 text-tertiary(variant default — 분할·「코드」·설정 칩의 꺼진 가지 색, 눌리면 toggle-on의 글자색이 이긴다). rounded-lg를 뿌리에서 크기로 옮겼다 — icon 크기가 부르는 icon-button(8px)을 코어 유틸리티가 덮는다(커스텀 @utility가 앞에 선다). 크기 둘을 더했다: icon(24px 아이콘 버튼 — 머리행의 분할, icon-button을 부른다)과 toolbar(22px 툴바 버튼 — 다이어그램 머리 줄의 「코드」, 7px 모서리 · 좌우 4px · 12px · 400, MermaidBlock의 toolbarButtonQuiet과 같은 값). 두 칸 토글(모드 전환·문서/원문)은 이 기본을 안 쓴다 — toggle-group의 segment 변형이 든다.
