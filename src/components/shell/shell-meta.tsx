...
/**
 * **셸 메타** — 「무리」의 나열이다(결정 3). 무리 하나 = 글리프 + **그 무리의 셸 수**이고,
 * 글리프는 「우리가 아는 한 그 셸이 지금 무엇인지」를 말한다.
 *
 ...
 *
 * **무리 순서는 마크가 붙은 것 먼저, 그 밖의 셸이 마지막이다**(결정 8). nav `Terminal`에서
 * 이 메타는 오른쪽 정렬이라 **오른쪽 끝에 서는 무리가 고정점**이다 — claude가 시작되면 새
 * 무리가 왼쪽으로 끼어들고 `⌨`는 제자리에 있다. 반대로 하면 이미 서 있던 숫자가 옆으로
 * 미끄러진다. work 행의 둘째 줄은 왼쪽 정렬이라 고정점이 반대편이지만 순서는 같이 간다 —
 * 한 조각이 두 자리에 서는 값이 「같은 어휘」이고, 자리마다 순서가 갈리면 그 말이 거짓이 된다.
 *
 * **`components/shell`에 사는 이유는 쓰는 자리가 둘이어서다**(결정 13) — nav `Terminal`의
 * 행 오른쪽 끝과 work 행의 둘째 줄. 이 저장소의 규칙은 「쓰는 자리가 하나면 그 파일로, 둘이면
 * 공용으로」이고, 세는 규칙과 규격(글꼴·간격·색)이 여기 하나에 있어야 그 둘이 앞으로도 안
 * 갈린다. **오른쪽 여백(`pr-[5px]`)만 한쪽의 것이다** — nav에서 구획 헤더의 개수와 같은 x에
 * 오른쪽 끝을 세우는 값이고, 왼쪽 정렬인 둘째 줄에서는 아무 일도 안 한다.
 *
...
export function ShellMeta({
  ...
  // 최상위 셸 없이 설 때 기대는 규칙이 이것이다. work 행은 여기 안 기댄다: 셸이 0개면 둘째
  // 줄이 **프로젝트 이름으로 갈라져야** 해서 같은 조건을 한 번 더 쓴다
