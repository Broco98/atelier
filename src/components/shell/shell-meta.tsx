...
export function ShellMeta({
  ...
  // 최상위 셸 없이 설 때 기대는 규칙이 이것이다. work 행은 여기 안 기댄다: 셸이 0개면 오른쪽
  // 메타의 **칸 자체**가 없어야 해서(칸이 제목과 떼는 여백을 든다) 같은 조건을 한 번 더 쓴다
  // (`WorkSectionList.tsx`) — **문턱을 고치면 두 자리다.**
    ...
    <span className="row-meta text-tertiary">
      ...
      {/* 「셸은 열려 있는데 우리가 아는 것은 안 돈다」 — 이 목록의 기본값이고, ⌘W로 닫을
          ...
          **색도 마크 무리와 같이 한 단 올린다.** 위 무리가 tertiary를 안 쓰는 근거(로고는
          ...
          (바로 위 문장이 「이 목록의 기본값」이라 부른다) work 행 메타가 통째로 3.0으로
          서던 자리다 — 두 줄 행을 연 판이 시작된 말이 「이 한 줄이 가독성이 안 좋다」였으니
          자리만 옮기고 이 갈래를 두고 오면 답이 안 된다. 그 대비를 L3가 계산해 잰다. */}
      {others > 0 && (
