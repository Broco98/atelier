  ...
  return (
    ...
    >
      {/* fixed inner width so text doesn't reflow while the width animates.
          ...
          **선 뒤에는 바깥 폭을 넘지 않는다** — 작업 패널 안쪽 열과 같은 장치다(WorkPanel 주석).
      ...
      <div
        className={cn(
          ...
          open
            ? "max-w-[calc(100%+1px)] transition-[opacity,max-width] opacity-100 delay-[0s,220ms] duration-[220ms,0s]"
...
function asideClass(open: boolean, dragging: boolean): string {
  return cn(
    ...
    // 900px 창) 여기 차례가 온다(`ShellTabs`의 `TAB_ROW_COLUMN`). 다른 화면의 본문은 자리를
    ...
    !dragging && "duration-[220ms] ease-panel",
