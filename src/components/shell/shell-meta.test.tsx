...
describe("셸 하나는 한 무리에만 들어간다", () => {
  ...
  it("무리 순서는 **마크가 붙은 것 먼저, 그 밖의 셸이 마지막**이다", () => {
    // 결정 8. nav `Terminal`에서 이 메타는 오른쪽 정렬이라 오른쪽 끝 무리가 고정점이다 — claude가 시작되면
...
describe("무리 하나 = 글리프 + 그 무리의 셸 수", () => {
  ...
  it("**무리는 하나도 빠짐없이** 바깥보다 한 단 진하다 — 대비 바닥이 그 이유다", () => {
    // 결정 15가 로고를 `currentColor`로 칠한 근거가 「대비 바닥 4.5를 저절로 넘는다」인데,
    // 바깥 색(tertiary)은 사이드바 배경에서 그 아래다(#8e8e97 대 #f7f7f9 ≈ 3.0). 바깥 색을
    // 그대로 물려받는 무리가 하나라도 있으면 결정 15의 근거가 그 무리에서만 거짓이 된다.
    //
    ...
    무리마다색(meta(1, ["claude"]), 1);
    // 그 밖의 셸만 — 이 갈래가 빠져 있었다.
    무리마다색(meta(1, ["vim"]), 1);
    // 둘이 함께 설 때도 둘 다.
    무리마다색(meta(3, ["claude"]), 2);
  });
