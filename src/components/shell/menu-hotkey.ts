...
//   ⌘B    어디서나 사이드바 — 화면을 안 탄다
//   ⌘T    works에서 셸 열기
//   ⌘K    어디서나 검색 팔레트 — 화면을 안 타고, **이 길이 그 키의 값의 절반이다**(결정 3):
//         문서를 읽다 검색하는 흐름이 정확히 프레임에 포커스가 있는 상황이다
//
...
//
// 남는 위험 하나를 적어 둔다: 이 다섯을 듣는 리스너 중 하나가 `preventDefault`를 안 부르게
 ...
 *
 * **둘 다 실어야 한다.** 리스너가 보는 값이 갈려 있다 — ⌘B·⌘T·⌘K·⌘1~9는 `code`를 보고
 * (`AppShell` · `shell-registry`), ⌘↩은 `key`를 본다(`WorksPage` · `ProjectsPage` ·
 * `ArchivePage`). 한쪽만 채우면 그 절반이 조용히 안 먹는다.
 *
 * **`code`를 보는 쪽이 늘어난 것은 우연이 아니다** — 한글 입력기가 켜지면 `key`가 자모로
 * 오므로, 새 키는 `code`로 보는 것이 이 저장소의 기본값이다.
 */
...
/**
 * 메뉴가 쏜 code로 창에 흘릴 keydown의 **속성**을 만든다. 수식키는 ⌘ 하나다 — 살린 것이
