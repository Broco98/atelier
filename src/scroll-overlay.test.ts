...
//
// **이 스캔이 못 보는 구르는 상자가 하나 있다 — `HtmlDoc`의 iframe 안이다.** 클래스 스캔은
// `src/**`의 소스만 보는데 거기서 구르는 것은 프레임 **안** 문서이고, 그 문서의 CSS는
// 발행본에서 그대로 떠 온 `ARTIFACT_SHELL_HEAD`가 전부다. **그 자리는 예외로 둔다** —
// 아티팩트와 1:1이 목적이라 네이티브 막대가 맞다(결정 2·9). 근거는 `SpecViewer.tsx`의
// `HtmlDoc` 머리말에 있다. 예외가 늘면 결정 32가 무너지므로, 여기 적히지 않은 예외는 없다.
