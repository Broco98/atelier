 ...
 *
 * **다만 지금 이 줄이 실물에서 설 일은 거의 없다.** 알림 플러그인의 데스크톱 구현은
 * `permission_state()`가 **늘 `Granted`를 돌려준다**(`desktop.rs`) — macOS 시스템 설정에서
 * 아틀리에의 알림을 꺼 둬도 앱은 그것을 못 읽는다. 그래도 이 줄을 두는 것은 판정이 진짜
 * API를 딛고 있어서다: 플러그인이 그 값을 실제로 물어보게 되는 날 화면이 저절로 따라온다.
 * 여기서 「거부됐을 것 같다」를 우리가 지어내지는 않는다(결정 3의 그 규칙).
 *
