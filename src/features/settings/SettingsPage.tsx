...
import { settingsItem, type SettingsPageKey } from "./pages";
 ...
 * 항목 경계에도 걸리는 것이다.
 */
function SettingsPage({ sidebarOpen, page }: { sidebarOpen: boolean; page: SettingsPageKey }) {
  return (
    <div className="flex min-h-0 min-w-0 flex-1">
      <main className="flex min-w-0 flex-1 flex-col">
        <PageHeader root="Settings" leaf={settingsItem(page).label} inset={!sidebarOpen} />
        <div className="min-h-0 flex-1 overflow-y-auto px-8 pb-10 scroll-quiet">
          <div className="flex max-w-[620px] flex-col gap-6">
            {page === "terminal" && (
              <SettingsFileGate>
                {(settings) => <TerminalSettingsPage initial={settings} />}
              </SettingsFileGate>
            )}
            {page === "notifications" && (
              <SettingsFileGate>
            ...
            {/* **읽기 실패의 게이트가 없다.** 이 항목이 고치는 것은 `~/.atelier/settings.json`이
                아니라 사용자의 claude·codex 설정이라, 우리 파일이 깨져 있다고 훅을 못 깔
                이유가 없다. */}
            {page === "hooks" && <AgentHooksPage />}
          </div>
        </div>
      </main>
    </div>
 ...
 * 알림을 저장하고 터미널로 옮겨 온 페이지가 새로 읽은 사본을 들어도, 옛 사본을 들어도 덮어쓰기가 없다.
 */
