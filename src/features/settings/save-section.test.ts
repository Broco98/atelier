...
function fakeFile(initial: unknown) {
  ...
  return {
    ...
    now: () => JSON.parse(text) as Record<string, unknown>,
    writes,
  };
}
...
describe("구획 저장은 최신 설정에 자기 칸만 덮는다", () => {
  it("알림을 저장한 뒤 옛 사본을 든 터미널 저장이 알림을 되돌리지 않는다", async () => {
    ...
    await save("terminal", { ...opened.terminal, fontSize: 16 } as Settings["terminal"]);

    expect(file.now().notifications, "알림 값이 옛 사본으로 되돌아갔다").toEqual({
      enabled: false,
      sound: false,
    });
    expect((file.now().terminal as Settings["terminal"]).fontSize).toBe(16);
  });
  ...
  it("터미널을 저장한 뒤 옛 사본을 든 알림 저장이 터미널을 되돌리지 않는다", async () => {
    ...
    await save("terminal", { ...opened.terminal, theme: "light" } as Settings["terminal"]);
    await save("notifications", { ...opened.notifications, sound: false });

    expect((file.now().terminal as Settings["terminal"]).theme, "터미널 값이 되돌아갔다").toBe(
      "light",
    );
  ...
  it("파일에 있던 모르는 최상위 키가 두 저장 뒤에도 남는다", async () => {
    ...
    await save("terminal", { ...opened.terminal, fontSize: 14 } as Settings["terminal"]);
  ...
  it("겹친 두 저장이 서로의 값을 안 지운다", async () => {
    ...
    await Promise.all([
      save("terminal", { ...opened.terminal, fontSize: 18 } as Settings["terminal"]),
      save("notifications", { enabled: false }),
    ]);

    expect(overlapped, "쓰기가 겹쳤다").toBe(false);
    expect((file.now().terminal as Settings["terminal"]).fontSize).toBe(18);
