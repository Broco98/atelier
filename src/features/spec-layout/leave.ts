...
/**
 * 편집기를 떠나는 이동을 붙잡아 묻는다. 떠나는 길은 머리의 뒤로, 사이드바 nav, 팔레트로 다른 곳 열기, 설정 nav의
 * 다른 항목, 그리고 히스토리를 되감는 셸의 뒤로·앞으로와 마우스의 옆 버튼이다. 모두 라우터의 이동이라 **라우터의
 * 막기 하나**(`useBlocker`)가 모두 받는다 — 길마다 물음을 걸면 한 길이 잊는 날 그 길로만 초안이 사라진다. 팔레트의
 * 「설정」은 이미 설정 안이면 이동이 아니다(`navigateGuardingSettings`) — 이동이 없으니 묻지도 않는다.
 *
 * 히스토리를 되감는 이동(pop)은 주소가 먼저 바뀐다 — 머물면 라우터가 히스토리를 편집기로 되돌린다. 뒤로 간 것은
 * 앞으로, 앞으로 간 것은 뒤로 되돌려야 하고, 그렇게 되돌리는 것은 `@tanstack/history` 1.162.2부터다(`go(-delta)`).
 * 1.162.0은 늘 앞으로 한 칸(`go(1)`) 되돌려, 앞으로 가려다 머물면 화면은 편집기인데 히스토리는 떠난 자리에 섰다.
 *
