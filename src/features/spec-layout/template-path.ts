...
// 때뿐이다.
//
// 이 파일은 이름만 짓는다. 경로가 폴더 안인지, `layout.json` 자신이 아닌지는 엔진의 검증(미리보기와 저장이 같은
// 한 벌)이 판정한다(결정 13) — 여기서 피하는 것은 그 검증에 걸리지 않는 이름을 **처음부터** 고르기 위해서다.
 ...
 * - 앞의 점은 뗀다 — 엔진은 점으로 시작하는 파일을 보지 않아(원자적 쓰기의 임시 파일 자리다) 저장의
 *   검증이 그런 템플릿 경로를 거절한다. 거절될 경로를 짓지 않으려 여기서 먼저 뗀다. 남는 이름이 없으면
 *   `template.md`다.
