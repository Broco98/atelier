...
// 행 사이 3px · 구획 머리 위 12px. (행 55px는 두 줄 행 시절의 값이고 지금 행은 한 줄 32px이다
// — `sidebar-active-band` 결정 14. 틈 판정은 재어 온 사각형만 보고 높이를 가정하지 않으므로
// 표는 그 수로도 같은 것을 잰다.)
//
...
// 순서나 구획이 바뀔 때뿐이다 — 제목·상태는 행 높이를 안 바꾼다(한 줄 32px로 못박혀 있다). 에이전트가
