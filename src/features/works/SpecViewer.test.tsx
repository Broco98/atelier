...
describe("PrettyView 참조 복사 버튼", () => {
  ...
  // **언어를 안 적은 코드블록도 블록이다.** 이 한 줄이 없어서 결함이 오래 살았다 — 위
  // `it.each`의 코드블록이 ```js라 늘 `language-js`가 붙었고, 언어 없는 블록만 지나는 길이
  // 아무 검사에도 안 걸렸다.
  //
  // 무엇이 났나: `code` 컴포넌트가 「`language-`가 없으면 인라인 코드」로 가르는데 언어를
  // 안 적은 **블록**도 className이 없다. 그래서 여러 줄짜리 블록이 인라인 껍데기(테두리·
  // 둥근 모서리·배경·0.88em)를 뒤집어썼고, 인라인 요소의 테두리가 줄마다 끊겨 **줄 사이에
  // 선이 그어진 것처럼** 보였다(실물에서 그렇게 났다).
  //
  // **둘을 함께 센다.** 「블록에 껍데기가 없다」만 보면 인라인 코드에서 껍데기를 통째로
  // 걷어도 초록이 된다 — 그쪽은 그 껍데기가 정답이다.
  it("언어 없는 코드블록은 블록으로, 인라인 코드만 껍데기를 쓴다", () => {
    const 블록 = render("```\n첫 줄\n둘째 줄\n```");
    expect(블록).toContain("<pre");
    expect(블록).not.toContain("rounded-[6px]");

    const 인라인 = render("문단 안 `코드` 하나");
    expect(인라인).toContain("rounded-[6px]");
    expect(인라인).not.toContain("<pre");
  });
