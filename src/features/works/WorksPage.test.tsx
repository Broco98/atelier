...
describe("WorksPage 터미널 탭", () => {
  ...
  // 결정 102. 정상 종료한 셸은 목록에서 스스로 빠지고(결정 48), 마지막 칸을 `×`로 닫은
  // 자리에서는 새 셸이 저절로 뜨지 않는다(판 02) — 셸이 0개인 화면이 실재하고, 그때 본문이
  // 그 자리를 낸다.
  //
  // **머리행 뒤부터 `</main>` 앞까지만 센다.** 사이드바 가지·패널에도 같은 이름의 `+`가
  // 있고, **탭 줄에도 이제 하나 선다**(결정 7) — 마크업 전체에서 세면 「본문에 있다」와
  // 「어딘가 있다」가 같아진다.
