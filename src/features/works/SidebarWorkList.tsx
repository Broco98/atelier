...
function WorkRow({
  ...
  return (
    <div
      ...
      className={cn(
        // **flex가 아니라 grid다.** 결정 1이 그것을 고른 이유 —— 메타와 핀이 **2열 같은 칸에
        // 겹쳐** 선다 —— 는 핀이 격자 밖으로 나가면서 **없어졌다**(핀 주석). 남은 둘(제목
        // `1fr` · 메타 `auto`)은 flex로도 같은 폭이 나므로 이제 이것은 필연이 아니라 선택이다.
        // 그래도 grid인 것은 **「2열」이 이 파일과 두 검사 층이 함께 쓰는 어휘**이기 때문이다 —
        // 폭이 한 픽셀도 안 바뀌는 개명에 세 파일을 물리지 않는다.
        //
        ...
        // 자리는 무리가 **실제로 설 때** 나고, 안 서면 그 폭은 제목의 것이다.
        //
        // **되찾는 폭**(L3 WebKit 실측 2026-08-30 · 사이드바 280px): 제목 상자가 셸 0개 행에서
        // 194.09 → **222.00px**. 셸이 있는 행은 그대로다 — 1무리 27.91px에 194.09px,
        // 2무리 56.81px에 165.19px. 되찾는 것이 셸 0개 행 하나뿐인 것은 예약이 바로 그
        // 행에서만 「없는 아이콘의 자리」였기 때문이다.
        //
        // **대신 감수하는 것 — 셸이 붙고 떨어질 때 제목이 끊기는 자리가 튄다.** 첫 셸이 서면
        // 27.91px, 무리가 둘이 되면 다시 28.90px 짧아진다. 예약이 있을 때는 첫 셸이 0px이고
        // 둘째 무리에서만 28.90px이었으니, 이 판이 새로 사들인 뜀은 **첫 셸의 27.91px**이다.
        // 결정 2·5가 사려던 것이 그 뜀이고, 그 값에 「없는 아이콘의 자리 28px」은 너무
        // 비쌌다는 것이 위 판단이다.
        //
        // **핀은 이 격자 밖에 선다** — `relative`가 그 기준을 여기서 든다(핀 주석). 안 빼면
        // 예약을 걷어도 제목이 **4px**밖에 안 돌아온다: `minmax(28px,auto)`의 `auto` 쪽
        // max-content가 핀 24px이라, 바닥 28을 걷는 순간 이번엔 핀이 트랙을 잡는다(28 → 24).
        // 셋을 실측으로 갈라 두면 — 핀만 격자 밖으로 +0px · 바닥만 걷어 +4px · **둘 다 +28px**.
        "group relative grid w-full shrink-0 grid-cols-[minmax(0,1fr)_auto] items-center rounded-[10px] pr-1 transition-colors",
    ...
    >
      ...
      {/* 평소 숨어 있다가 hover에만 뜬다(결정 85) — 고정 여부는 구획이 이미 말하고,
          ...
          **격자 밖에 선다 — 행의 `relative`에 대고 `absolute right-1`이다.** 결정 1은 이것을
          메타와 같은 2열 1행에 놓아 칸 폭을 `max(메타, 핀)`으로 겹치게 했는데, 그러면 이
          24px이 **셸이 0개인 행에서도 늘 예약**이라 「아이콘 생기면 그때 가변된다」와 정면으로
          어긋난다(행 상자 주석). 격자에 남긴 채 `opacity-0`이면 자리는 늘 잡혀 있고,
          `display:none`으로 걷으면 hover마다 칸이 0↔24로 뛰어 제목이 좌우로 밀린다 —
          **둘 다 나쁘다.** 밖으로 빼면 어느 쪽도 아니다 — 실측 hover 밀림이 셸 0·1무리·2무리에서
          **0.00px**이고(제목 상자 222.00 / 194.09 / 165.19px, 쉴 때와 hover가 같은 수),
          쉴 때도 2열은 메타의 폭 그대로다.

          **겹침은 그대로다.** `right-1`이 행의 패딩 상자를, 메타는 `pr-1` 안쪽 2열 끝을 잡아
          **둘 다 268px에서 끝난다**(실측). 핀 상자는 2열 폭과 무관하게 244~268px 고정이다.

          **`col-start-2 row-start-1`은 걷었다** — `absolute`면 격자 배치가 위치를 안 정한다.
          걷기 전후로 셸 0·1무리·2무리 어느 행에서도 핀 상자가 244~268px 그대로였다(실측).
          `shrink-0`도 같이 걷었다: 격자 밖이라 flex 항목이 아니고, `icon-button`이 이미
          `flex-shrink: 0`을 든다.

          **핀 글리프가 서는 자리는 제목의 페이드 띠와 정확히 같다.** 글리프 12px이
          250~262px, 제목 상자 오른쪽 끝이 셸 0개 행에서 262px이고 `--title-fade`가 12px이라
          띠도 250~262px다. 우연이 아니라 양쪽이 같은 10px에서 재기 때문이다 — 제목 상자는
          행의 `pr-1`(4) + 이름 버튼의 `pr-1.5`(6), 핀 글리프는 `right-1`(4) + 24px 상자
          안의 여백(6). 그래서 이 겹침이 덮는 것은 **이미 흐려 못 읽는 자리**뿐이고, 넘치는
          제목은 마퀴가 「넘침 + 페이드」만큼 밀어 마지막 글자를 250.23px에 세운다 — 글리프에
          **닿되 안 겹친다**(실측). 남는 갈래는 **안 넘치면서 끝이 그 12px 안에 드는 제목**
          하나이고, 그때 끝 글자 위에 핀이 얹혀 뭉개져 보인다. 결정 12가 「거의 꽉 찬 제목은
          끝 글자가 살짝 옅어진다」로 이미 감수한 그 경우에 한 겹이 더 붙은 것이고,
          **셸이 하나라도 있으면 제목 상자가 234.09px에서 끝나 16px이 남아** 이 갈래는
          셸 0개 행에만 있다. 배경으로 가리지 않는 것은 결정 31이다 — `icon-button-tint`는
          배경을 안 켠다(실측 `background-color: rgba(0, 0, 0, 0)`).
          ...
          `이름 버튼 → 핀 → 메타`라 그대로 먹는다 — **`absolute`는 DOM 순서를 안 바꾸므로
          이 관계는 격자 밖으로 나와도 그대로다.** */}
      <button
        type="button"
        aria-label={`${work.title} 고정`}
        aria-pressed={work.pinned}
        onClick={() => onTogglePin(work)}
        className="peer icon-button-tint absolute right-1 text-tertiary opacity-0 outline-none focus-visible:opacity-100 focus-visible:ring-2 focus-visible:ring-ring/50 group-hover:opacity-100"
      >
        <Pin
          className="size-3"
          strokeWidth={1.8}
      ...
      {/* **셸 메타 — 셸이 하나라도 있으면 선다**(결정 3). 셸이 0개인 work의 행에는 아무것도
          ...
          **이제 2열에 서는 것은 이것 하나다.** 핀이 격자 밖으로 나가(핀 주석) 칸 폭이
          `max(메타, 핀)`이 아니라 **메타의 폭 그대로**다 — 1무리 27.91px · 2무리 56.81px
          (실측). 그래도 둘은 여전히 같은 자리에 겹쳐 보인다: 핀의 `right-1`과 이 칸의 끝이
          둘 다 268px이라서다. `justify-self-end`는 그래서 걷었다 — 트랙이 `auto`라 칸 폭이
          곧 이 상자의 폭이고, 끝에 붙이든 늘리든 같은 상자가 난다(걷기 전후 실측
          240.09~268px 동일). `col-start-2 row-start-1`은 남는다: 이것이 이 칸의 유일한
          자동 배치 대상이 되어 우연히 같은 자리로 가는 것과, **2열에 선다고 적는 것**은 다르다.

          hover하면 메타가 **투명해진다**(`hidden`이 아니다 — `display:none`은 칸 폭 계산에서
          빠져 2열이 **0으로** 무너지고, 핀이 격자 밖이라 그 폭을 아무도 안 받아 hover마다
          제목이 27.91px씩 뛴다. 핀이 격자 안에 있던 때는 24px로 줄었으니, 밖으로 나온 지금
          이 규칙은 **더** 중요해졌다). `visibility:hidden`도 아니다: 셸 수가 마우스 위치에
          따라 있다 없다 하는 정보가 되면 안 된다(결정 6). 트랜지션은 안 건다 — 옆 행으로
          옮겨 갈 때 두 페이드가 겹쳐 미끄러져 보인다(icon-button-tint가 opacity를 뺀 것과
          같은 이유).
          ...
          **표시 전용이다**(결정 5). 무리 하나가 셸 여럿을 접으므로 무리와 셸이 1:1이 아니고,
          ...
          (핀이 `absolute`가 되면서 그리는 순서로는 이제 핀이 위지만, 그 순서에 기대지
          않는다 — 누를 것이 없다는 이유가 그것과 무관하게 그대로다.) */}
      {shellCount > 0 && (
        <div
