...
// 배경(선택·hover)은 바깥 상자가 갖고, 여백은 안쪽 것들이 품는다: 바깥이 가진 padding은
// 두 버튼 어디에도 속하지 않아 배경은 덮이는데 눌러도 아무 일이 없는 죽은 자리가 된다.
// 남는 것은 오른쪽 끝 `pr-[10px]`뿐이고 그건 핀을 행 가장자리에서 띄우는 값이다.
  ...
  return (
    <div
      ...
      className={cn(
        ...
        //
        // **여백은 여전히 안쪽 것들이 품는다.** 바깥 상자가 padding을 가지면 그 띠는 두
        // 버튼 어디에도 안 속해 배경은 덮이는데 눌러도 아무 일이 없는 죽은 자리가 된다 —
        // 그래서 위 8은 이름 버튼의 `pt-2`, 아래 7은 둘째 줄의 `pb-[7px]`, 왼쪽 9는 이름
        // 버튼의 `pl-[9px]`이 든다. 오른쪽 10만 여기 있다: 그 자리에 서는 것이 핀뿐이고,
        // 핀은 자기 오른쪽에 여백을 두는 대신 칸 끝에 붙기 때문이다(`justify-self-end`).
    ...
    >
      <button
        ...
        // 이 버튼이 **눌리는 자리**이기 때문이다: 트랙만 26px이고 버튼이 18px이면 위 8px이
        // 배경만 있고 눌리지 않는 띠가 된다.
        className="col-start-1 row-start-1 flex h-[26px] min-w-0 items-center gap-(--glyph-gap) pl-[9px] pr-1.5 pt-2 text-left"
      >
      ...
      <div
        ...
        className="col-span-2 row-start-2 flex min-w-0 items-center overflow-hidden pb-[7px] pl-[32px] pt-1 text-[11.5px] text-muted-foreground"
      >
        {shellCount > 0 ? (
          /* **셸 메타 — 셸이 하나라도 있으면 선다**(결정 3). 「없음」은 숫자로 말하지
             ...
             **표시 전용이다**(결정 5). 무리 하나가 셸 여럿을 접으므로 무리와 셸이 1:1이
             아니고, 누르면 어느 셸로 갈지 정해지지 않는다. `pointer-events-none`은 그
             사실을 구조로 적어 둔 것이다 — 겹쳐 서던 핀의 클릭을 가로채던 사정은
             자리가 갈리면서 사라졌지만, 「누를 것이 없다」는 그대로다. */
          <div data-shells={work.slug} className="pointer-events-none flex min-w-0 items-center">
            {shellMeta}
          </div>
        ) : (
