 ...
 *
 * **입구가 여럿이어도 메뉴는 하나다.** `+`가 서는 자리가 셋이다 — 사이드바 가지와
 * 셸 0개인 터미널 본문(둘 다 `ShellList`의 글자 있는 행)과, **탭 줄의 아이콘 하나**
 * (`ShellTabs` — 결정 7). 마지막 것은 규격이 아주 달라(폭 없는 아이콘 vs 글자 있는 행)
 * 한때 그것 때문에 **열리는 것만** 여기 모았고, 그 줄이 걷혔다 돌아온 지금 그 이유가
 * 그대로 다시 선다.
 *
