...
describe("codex 어댑터가 페이로드를 정규 이벤트로 접는다", () => {
  ...
  // 하던 말이 곧 맥락이고, 그 자리를 비우면 말을 싣는 자리(카드의 말 칸·띠)가
  // 이유 없이 빈다.
...
describe("훅이 말한 셸에서는 OSC·벨·출력이 아무것도 못 바꾼다", () => {
  ...
  // 도는 것은 이미 없으므로, 이 값이 안 실리면 행의 오른쪽 메타가 마크를 낼 재료가 없다.
...
// **행이 그리는 것은 값 하나가 아니라 한 줄이다**(#203). 레인의 점·스피너는 화면값이 정하지만
// 오른쪽 메타와 카드는 그 값을 **낸 셸**의 시각과 마크와 말까지 든다 — 그 넷이 다른 함수에서 나오면
...
describe("행이 읽는 한 줄", () => {
  ...
  // 것」에서만 뽑으면 초록 행의 오른쪽 메타는 **늘** 경과뿐이고, 티켓과 스펙이 못박은
  ...
  // **`topSignal`이 이 함수를 딛는다.** 우선순위를 아는 자리가 둘이면 레인과 오른쪽 메타가
  // 다른 셸을 고를 수 있다.
...
// 그렇게 빠졌다. 그래서 터미널 트리를 통째로 읽어 **시간을 아는 파일의 목록이 아래 넷과
// 정확히 같은지**를 본다(옆의 `상태값을만지는파일`과 같은 모양). 「축에 없다」가 아니라
// 「이 넷뿐이다」라서, 새 파일이 시계를 들이면 여기서 이름을 대며 터지고 스캔이 통째로
...
// 들어오면 결정 2·3이 깨진 것이다 — 넷 다 상태 축 밖의 이유로 시간을 안다.
//
...
const 시간을아는파일 = [
  ...
  "ShellTabs.tsx",
  // `+` 메뉴가 열린 **다음 프레임**에 첫 줄에 포커스를 주는 자리(`requestAnimationFrame`, S10).
  // **메뉴 부품의 박자지 상태의 시간이 아니다** — 옮기는 것은 DOM 포커스 하나이고 셸 상태는 안 만진다.
  "ShellPicker.tsx",
];

it("터미널에서 시간을 아는 파일은 넷뿐이다 — 상태 축엔 시계도 타이머도 없다", () => {
