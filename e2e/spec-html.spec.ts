...
// 프레임에 포커스가 들어가면 앱 단축키가 죽는다(이슈 #153). **넷은 되살아났다** — ⌘1~9·
// ⌘B·⌘T·⌘↩을 네이티브 메뉴가 대신 받는다(`components/shell/menu-hotkey.ts`). 그 배선은
// 진짜 OS 메뉴가 있어야 도는 것이라 **어느 층도 못 잰다**: 이 층의 브라우저에는 메뉴가 없다.
// 여기서 재는 것은 남은 셋(⌃Tab·⌘W·⇧⇧)을 말하는 카드가 서고 사라지는 것과, 그것이
// **덮개가 아니라는 것**이다.
//
...
test("프레임에 포커스가 있는 동안만 「단축키가 안 먹는다」가 서고, 덮개는 아니다", async ({
  ...
  await expect(hint).toContainText("단축키 셋이 지금 안 먹어요");
  await expect(hint).toContainText("문서 바깥을 한 번 클릭하면");
  // **어느 셋인지까지 센다.** 메뉴가 넷을 되살렸으므로 이 목록은 이제 줄어든 것이고,
  // 「안 먹는다」만 세면 넷이 도로 죽어도 초록이 된다.
