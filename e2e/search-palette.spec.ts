...
/**
 * `landing`은 무선택 주소(`/maison/rooms`)가 정규화로 고르는 Room — 첫 줄이다(초안이어도 안
 * 건너뛴다, UI개선 결정 6). `room`은 팔레트의 Maison 답(`MAISON_SEARCH_HITS`)이 싣는 **문서를
 * 가진 둘째**다. 둘이 한때 같은 Room이었다(정규화가 초안을 건너뛰던 때).
 */
const [landing, room] = ROOMS;
