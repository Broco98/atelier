//! 종료 확인(결정 14·15 · 스펙 §8). 앱을 끄는 길은 **프런트가 사람에게 물은 뒤에만** 열린다.
//!
//! 이 파일은 **플랫폼 cfg 밖이다**(S15). 빨간 버튼의 창 닫기(`lib.rs`의 `on_window_event`)와
//! 11이 붙일 macOS 델리게이트 훅이 같은 이벤트 이름과 같은 「확인됨」 표시를 쓰므로, 여기가
...
///
/// 세우는 이유: 종료가 진행되는 사이 창 닫기나(11의) `terminate:`가 끼어들면 그 길이 다시 막고
/// 물어, 「종료」를 누른 사람 앞에 창이 한 번 더 뜨거나 종료가 멈춘다.
pub fn confirm() {
...
mod tests {
    ...
    /// 창을 닫고, 확정 뒤에도 안 서면 `quit_app`이 부른 종료가 다시 창 닫기에 막힐 수 있다.
    ///
