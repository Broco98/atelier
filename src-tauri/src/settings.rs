...
//! 성격에서 나온다.
//!
//! **감수한 것 — 설정이 두 곳에 갈린다.** `localStorage`에 이미 다섯이 산다(섹션 접힘 셋 ·
//! 패널 열림 · 패널 폭)이고 원칙까지 주석에 있다(`SidebarWorkList.tsx`의 `PINNED_OPEN_KEY`
//! 머리말, 「설정은 영속, 위치는 세션」). 선은 이렇게 긋는다 — 접힘·폭·패널 열림은 **화면이
//! 어떻게 놓였나**(위치에 가까운 UI 상태), 글꼴·크기·테마는 **취향**(진짜 설정). 기존
//! 다섯은 옮기지 않는다.
