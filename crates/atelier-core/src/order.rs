...
/// 다음 저장이 그 손질을 덮어쓰기 때문이다. 순서 파일에서는 그 위험을 **쓰는 쪽이 좁힌다**:
/// 눕는 것은 이 판정 하나이고 파일은 안 건드리며, 지우기(`forget_in_order`)는 빈 순서에서 뺄
/// slug가 없으니 아예 안 쓴다. 깨진 파일을 덮는 길은 **옮기기**(`works.rs`의 `reorder` —
/// 고정 토글도 탄다) 하나다. 그것은 사람이 화면에서 **보고 있는** 순서(깨진 파일이 빈 순서로
/// 누운 결과)를 굳혀 적으므로, 잃는 것은 이미 화면에 없던 손질이고 그 사실은 이 줄이 남긴
/// stderr 한 줄이 들고 있다. 그러면서 목록 읽기는 검색이 글자마다 부르는 자리라, 실패로 말하면
/// 파일 한 장 때문에 사이드바도 팔레트도 못 뜬다 — 그쪽 비용이 더 크다. 조용히 비면 「순서가 왜
/// 풀렸지」에 답할 자리가 없어 한 줄 남긴다.
///
