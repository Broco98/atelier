//! 「마지막으로 연 work」 이력 한 장 — `~/.atelier/recent.json` (결정 13).
//!
//! **코어가 소유한다.** 이 값을 읽어 순서를 정하는 것이 검색이고(결정 11), 순서 규칙은
//! 코어 밖으로 안 나간다 — 프런트가 정렬된 slug 배열을 건네게 되는 순간 정책이 바깥으로
//! 샌다. `localStorage`가 아니라 파일인 것도 같은 줄기다: MCP·CLI로 붙는 에이전트가
//! 언젠가 「마지막으로 보던 work」을 물을 수 있어야 한다(결정 13).
//!
//! **여기 사는 것은 파일 한 장의 읽기·쓰기뿐이다.** 무엇을 「열었다」로 셀지는 화면이 알고
//! (결정 14), 그 순서로 목록을 세우는 것은 검색이 한다.
...
///
/// **길이에 상한을 두지 않는다**(결정 15). 줄 하나가 slug 하나라 파일이 커질 일이 없고,
...
mod tests {
    ...
    /// 결정 25. **항목이 객체다** — 문자열 배열이면 앞으로 붙을 frecency 값이 못 자란다.
    ...
    /// 결정 24. 깨진 파일은 **빈 이력으로 눕는다** — 검색이 글자마다 부르는 자리라 파일
