...
///   있던 폴더에 쓰다가 실패하면 `layout.json`은 전과 같지만, 먼저 쓴 템플릿은 새 본문인 채 남는다 —
///   앞 레이아웃도 가리키는 템플릿이면 앞 레이아웃이 그 새 본문을 준다(저장 전체가 원자적이지는 않다).
...
pub fn save_layout(
    ...
    if let Err(e) = write_templates_then_layout(&folder, &layout, templates) {
        ...
        // 넘기므로 흔한 경우다). 앞 레이아웃이 모르는 경로만 레이아웃이 모르는 파일로 남는다.
        if first {
