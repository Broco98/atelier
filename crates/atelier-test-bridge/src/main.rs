...
use atelier_core::{archive_dir, data_root, projects_dir, works_dir, ProjectPatch};
...
const HANDLERS: &[(&str, Handler)] = &[
    ...
    ("search", |a| {
        ok(atelier_core::search(
            // **반드시 코어의 데이터 루트다.** 다른 값을 넘기면 L4가 sandbox 밖(진짜 홈)을
            // 봐서, 관통 검사가 개발자의 실제 사용 기록에 따라 초록·빨강을 오간다.
            &data_root(),
            &text(a, "query")?,
            &destinations(a)?,
        ))
    ...
    ("touch_recent_work", |a| {
        ok(atelier_core::touch_recent_work(&data_root(), &text(a, "slug")?))
    }),
