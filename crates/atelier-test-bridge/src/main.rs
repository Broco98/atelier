...
mod tests {
    ...
    ///
    /// `mode`가 무엇을 담는지(인자에서 왔는가)까지는 이 검사가 못 본다 — 그 몫은 앱 쪽
    /// 단위 테스트(`commands.rs`의 `모드를_안_준_호출은_atelier로_간다`)와 다리의 계약
    /// 테스트(`tests/mode_contract.rs`)가 **실행**해서 든다. 아래 검사는 그 단위 테스트
    /// 하나가 명령 전부를 덮는다는 전제(「펴는 자리가 하나다」)를 붙든다.
    #[test]
    ...
    /// **`Option`을 펴는 자리는 하나다** (#181). 위 검사는 루트 함수가 `mode`를 받는지만
    /// 세므로, 명령이 제 손으로 `let mode = mode.unwrap_or(…)`를 적는 변형은 그대로 통과한다.
    /// 그러면 「없으면 Atelier」가 열세 벌이 되고, 하나가 다른 값으로 눕는 날 그 명령만
    /// 저쪽 세계를 읽는다 — 화면과 데이터가 어긋난 채로 조용히 돈다.
    ///
    /// **무엇으로 펴는지는 여기서 안 본다.** 그 값은 `commands.rs`의 단위 테스트가 함수를
    /// 직접 불러 잰다. 여기가 드는 것은 그 테스트 하나가 명령 전부를 덮게 하는 전제뿐이다.
    ///
    /// **파싱이 없다.** 서명에 선 `Option<Mode>`의 수와 그것을 넘기는 호출의 수를 견줄
    /// 뿐이다. `+ 1`은 **펴는 함수 자신의 서명**이다 — 그 함수가 사라지거나 이름이 바뀌면
    /// 호출 수가 0이 되어 「표식이 낡았다」로 먼저 터진다.
    ///
    /// contract 티켓(#187)이 `Option`을 걷어 낼 때는 이 검사도 함께 사라진다 — 그때는
    /// 서명 수가 0이라 여기서 빨개지고, 그것이 「함께 지워라」라는 신호다.
    #[test]
    fn 명령이_모드_기본값을_한_자리에서만_정한다() {
        let source = APP_SOURCES
            .iter()
            .find(|(f, _)| *f == "commands.rs")
            .expect("commands.rs가 앱 소스 표에서 사라졌다")
            .1;
        let handoffs = source.matches("or_atelier(mode)").count();
        assert!(
            handoffs > 0,
            "commands.rs가 'or_atelier'에 mode를 한 번도 안 넘긴다 — 표식이 낡았다"
        );
        assert_eq!(
            source.matches("mode: Option<Mode>").count(),
            handoffs + 1,
            "commands.rs에서 'Option<Mode>'를 받는 자리와 그것을 넘기는 자리가 어긋났다 \
             — 명령 하나가 기본값을 제 손으로 정한다"
        );
    }
